// Assign2.c
// Nicholas Culmone
// Creates a ppm file based on the parameters given by the user. This file contains
// four squares in each of the corners, where each one is an entered parameter, and
// a diamond in the middle. The default size is 1000 X 1000, and creates the image
// using 10 children. But this can be altered by entering two more parameters at the
// command line, one being the side length, and the other the number of children
// (the length and number of children are rounded accordingly so the image is created
// correctly with no gaps in it)
// Note: takes a while to run on default settings
